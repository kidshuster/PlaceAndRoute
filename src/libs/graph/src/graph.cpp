...
namespace NetToMag
{
    ...
    void PrintToMagic(cubeGrid wGrid, string argw)
    {
        time_t t = time(0);
        long int now = static_cast<long int> (t);
        ofstream outMagic(argw);
        outMagic    << "magic"              << endl
                    << "tech scmos"         << endl
                    << "timestamp"
                    << now << endl;
        outMagic    << "<< pdiffusion >>"   << endl; 

        outMagic    << "<< polysilicon >>"  << endl;
        for(int i=0; i<wGrid.size(); i++)
            for(int j=0; j<wGrid[i].size(); j++)
                for(int k=0; k<wGrid[i][j].size(); k++)
